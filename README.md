...
- [AmiSlicer](#amislicer)
  ...
  - [13. 安全に関する注意](#13-安全に関する注意)
  - [14. ライセンス / License](#14-ライセンス--license)
...
┌─────────────────────────────────────────────────────────────────┐
│ メニューバー: File | Help                                        │
│  File: Open STL… / Save Settings… / Load Settings…              │
│        Save as Default / Reset to Default                       │
│        Reset to Factory Defaults / Quit                         │
├───────────────────────┬─────────────────────────────────────────┤
│ STLファイル            │                                         │
│ ├ Open STL...         │     2D Preview / 3D Preview             │
│ ├ メッシュ情報         │                                         │
│ └ モデル回転           │     ┌─────────────────────────┐         │
│                       │     │                         │         │
│ ┌─Machine─Print─Wave─┐│     │    プレビュー描画領域     │         │
│ │                     ││     │                         │         │
│ │  設定入力エリア      ││     └─────────────────────────┘         │
│ │                     ││                                         │
│ └─────────────────────┘│     Layer: 0 / 21  [━━━━━━━━━]         │
...
2. `.stl` ファイルを選択
   Select a `.stl` file
3. 読み込み後、以下の情報が表示されます / After loading, the following info is shown:
   * **Size X/Y/Z** — モデルのバウンディングボックスサイズ (mm)
   * **Faces** — 三角面の数
   * **Manifold** — Yes（閉じた形状）/ No（非マニフォールド、断面に問題が出る可能性）
4. 自動的にスライスとプレビューが生成されます
   Slicing and preview are generated automatically
...
| 使用する3Dプリンタのサイズを確認して入力 |                    |                                                                                                              |
| Nozzle Diameter                          | 0.4 mm             | ノズル径                                                                                                     |
| 使用するノズル径を入力                   |                    |                                                                                                              |
| Filament Diameter                        | 1.75 mm            | フィラメント径                                                                                               |
| 使用するフィラメント径を入力             |                    |                                                                                                              |
| Start G-code                             | ホーミング・加熱等 | 出力先頭のカスタムG-code<br />⚠️今お使いのスライサーソフトからコピーするなどして、正しく入力してください。 |
| End G-code                               | 冷却・退避等       | 出力末尾のカスタムG-code<br />⚠️今お使いのスライサーソフトからコピーするなどして正しく入力してください。   |
...
> 💡 Waveタブ下部に **Print Settings (synced)** グループがあり、 **Layer Height** ・ **Line Width** ・ **Nozzle Temp** ・ **Flow Multiplier** ・**Perimeter Speed** をWaveタブからも確認・変更できます。これらの値は波を設定する際にタブを切り替えずに確認できるようにしています。Printタブと双方向同期しており、どちらで変更しても反映されます。
...
| **Apply sine wave to fill lines** | OFF                  | 底面充填ライン波を適用                                  |
| **Bottom Fill Pitch**             | 0.4 mm               | 充填ライン間隔 / Fill line spacing                      |
...
> 💡 Bottom Layers は Fill bottom のON/OFFに関係なく常に有効です（0に設定するとbottomレイヤーなし）。Wave Start Layer は Bottom Layers からの相対値で指定します（例: Bottom Layers=3, Wave Start Layer=0 の場合、レイヤー3から波が開始されます）。Alternate phase が有効な場合、位相はbottomレイヤーからwaveレイヤーまで連続的に交互反転します。
...
2. 修正結果がプレビューに反映されます
3. 修正前の状態に戻したい場合
   **「Restore Self-Intersections」** ボタンが表示されるのでクリックします

> 💡 修正後もまだ自己交差が残る場合は、波の振幅（ **Amplitude** ）を小さくする、波の数（Wave Count）を変更する、使用するSTLファイル自体を変更するなど調整を検討してください。
...
> 💡  Preview G-Code…を押すことでG-Codeを保存前に確認することができます。
...
* **「Save Settings…」** をクリック or メニュー **File → Save Settings…** (`Ctrl+S`)
* JSON形式で Machine / Print / Wave の全設定を保存
* ファイル名はデフォルトでSTLファイルと同じ名称を候補で入力しています。

### 9.2 読み込み

* **「Load Settings…」** をクリック or メニュー **File → Load Settings…** (`Ctrl+Shift+O`)
* 設定JSONファイルをウインドウにドラッグ＆ドロップすることでも読み込むことが可能です。
...
* Amplitude を小さくする
* モデルの凹凸が激しい部分では修正に限界があるので、STL ファイルを変更することをお勧めします。
...
* 本ソフトウェアは **3Dプリンタのヒーター温度やモーターを制御を制御するG-codeを生成**します。十分に注意して使用してください。
